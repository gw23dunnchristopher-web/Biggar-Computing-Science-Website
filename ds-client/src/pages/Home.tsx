...
import { Database, PlusCircle, DatabaseBackup, Trash2 } from 'lucide-react';
...
export function Home() {
  ...
  const doDeleteDatabase = async () => {
    ...
    setIsDeleting(true);
    try {
      await apiFetch(`/api/databases/${deleteConfirm.dbId}`, { method: 'DELETE' });
  ...
  return (
    <div className="min-h-screen w-full bg-[#f3f2f1] flex flex-col overflow-hidden text-sm">
      <div className="h-12 bg-[#C42B1C] flex items-center px-6 text-white font-semibold text-lg shadow-md z-10">
