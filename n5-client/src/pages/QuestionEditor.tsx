...
import { ArrowLeft, Plus, Trash2, Save, ChevronDown, ChevronUp, Upload, PlusCircle, X, MoveUp, MoveDown, Type, Image, Code, FileText, AlignLeft, AlignCenter, AlignRight, Eye, Layers, Ungroup, Settings } from "lucide-react";
...
interface AdditionalPaperOption {
  id: string;
  name: string;
  isPublished: boolean;
}
...
export default function QuestionEditor() {
  ...
  useEffect(() => {
    ...
    if (token) {
      fetch("/api/teacher/additional-papers", {
        ...
        .then(data => setAdditionalPapers(data))
        .catch(() => {});
    }
  }, []);
  ...
  const updateContentBlock = (subIndex: number, blockIndex: number, field: keyof ContentBlock, value: string | boolean) => {
    const subQ = formData.subQuestions[subIndex];
    const blocks = [...(subQ.contentBlocks || [])];
  ...
  const updatePseudocodeLine = (subIndex: number, blockIndex: number, lineIndex: number, field: "lineLabel" | "content", value: string) => {
    const subQ = formData.subQuestions[subIndex];
    const blocks = [...(subQ.contentBlocks || [])];
    const block = blocks[blockIndex];
  ...
  const addPseudocodeLine = (subIndex: number, blockIndex: number) => {
    const subQ = formData.subQuestions[subIndex];
    const blocks = [...(subQ.contentBlocks || [])];
    const block = blocks[blockIndex];
  ...
  const removePseudocodeLine = (subIndex: number, blockIndex: number, lineIndex: number) => {
    const subQ = formData.subQuestions[subIndex];
    const blocks = [...(subQ.contentBlocks || [])];
    const block = blocks[blockIndex];
  ...
  const updateSubPartContentBlock = (subIndex: number, partIndex: number, blockIndex: number, field: keyof ContentBlock, value: string | boolean) => {
    const subQ = formData.subQuestions[subIndex];
    const parts = [...(subQ.subParts || [])];
    const part = parts[partIndex];
    const blocks = [...(part.contentBlocks || [])];
  ...
  const updateSubPartPseudocodeLine = (subIndex: number, partIndex: number, blockIndex: number, lineIndex: number, field: "lineLabel" | "content", value: string) => {
    const subQ = formData.subQuestions[subIndex];
    const parts = [...(subQ.subParts || [])];
    const part = parts[partIndex];
    const blocks = [...(part.contentBlocks || [])];
  ...
  const addSubPartPseudocodeLine = (subIndex: number, partIndex: number, blockIndex: number) => {
    const subQ = formData.subQuestions[subIndex];
    const parts = [...(subQ.subParts || [])];
    const part = parts[partIndex];
    const blocks = [...(part.contentBlocks || [])];
  ...
  const removeSubPartPseudocodeLine = (subIndex: number, partIndex: number, blockIndex: number, lineIndex: number) => {
    const subQ = formData.subQuestions[subIndex];
    const parts = [...(subQ.subParts || [])];
    const part = parts[partIndex];
    const blocks = [...(part.contentBlocks || [])];
  ...
  const handleDrawingBackgroundUploadSubPart = async (e: React.ChangeEvent<HTMLInputElement>, subIndex: number, partIndex: number) => {
    const file = e.target.files?.[0];
    if (!file) return;
  ...
  const updateScenarioContentBlock = (blockIndex: number, field: keyof ContentBlock, value: string | boolean) => {
    const blocks = [...(formData.scenario?.contentBlocks || [])];
    blocks[blockIndex] = { ...blocks[blockIndex], [field]: value };
    setFormData(prev => ({
      ...prev,
  ...
  const updateScenarioPseudocodeLine = (blockIndex: number, lineIndex: number, field: "lineLabel" | "content", value: string) => {
    const blocks = [...(formData.scenario?.contentBlocks || [])];
    const block = blocks[blockIndex];
    if (block.pseudocodeLines) {
      const newLines = [...block.pseudocodeLines];
  ...
  const getNextLineNumber = (lines: PseudocodeLine[]): string => {
  ...
  const addScenarioPseudocodeLine = (blockIndex: number) => {
    const blocks = [...(formData.scenario?.contentBlocks || [])];
    const block = blocks[blockIndex];
    const lines = block.pseudocodeLines || [];
    const nextNum = getNextLineNumber(lines);
  ...
  // Paste handlers for images
  const handleImagePaste = (e: React.ClipboardEvent, callback: (base64: string) => void) => {
  ...
  const handleContentBlockImagePaste = (e: React.ClipboardEvent, subIndex: number, blockIndex: number) => {
    handleImagePaste(e, (base64) => updateContentBlock(subIndex, blockIndex, "content", base64));
  };

  const handleSubPartContentBlockImagePaste = (e: React.ClipboardEvent, subIndex: number, partIndex: number, blockIndex: number) => {
    handleImagePaste(e, (base64) => updateSubPartContentBlock(subIndex, partIndex, blockIndex, "content", base64));
  };

  const handleScenarioContentBlockImagePaste = (e: React.ClipboardEvent, blockIndex: number) => {
    handleImagePaste(e, (base64) => updateScenarioContentBlock(blockIndex, "content", base64));
  };

  // Drag and drop handlers for images
  const handleImageDrop = (e: React.DragEvent, callback: (base64: string) => void) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const handleContentBlockImageDrop = (e: React.DragEvent, subIndex: number, blockIndex: number) => {
    handleImageDrop(e, (base64) => updateContentBlock(subIndex, blockIndex, "content", base64));
  };

  const handleSubPartContentBlockImageDrop = (e: React.DragEvent, subIndex: number, partIndex: number, blockIndex: number) => {
    handleImageDrop(e, (base64) => updateSubPartContentBlock(subIndex, partIndex, blockIndex, "content", base64));
  };

  const handleScenarioContentBlockImageDrop = (e: React.DragEvent, blockIndex: number) => {
        ...
        <Card>
            ...
            <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                        ...
                        onValueChange={(val) => {
                            if (val === "none") {
                                setFormData(prev => ({
                                    ...
                                    year: prev.year === 0 ? new Date().getFullYear() : prev.year,
                                }));
                            } else {
                                setFormData(prev => ({
                                    ...prev,
                    ...
                    >
                        <SelectTrigger data-testid="select-additional-paper">
                            <SelectValue placeholder="Select additional paper" />
                        </SelectTrigger>
                        <SelectContent>
                            <SelectItem value="none">None (not an additional paper)</SelectItem>
                            {additionalPapers.map(p => (
                                <SelectItem key={p.id} value={p.id}>{p.name}{p.isPublished ? "" : " (Draft)"}</SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
                </div>
                ...
                <div className="space-y-2">
                    <Label>Year {(formData.isPractice || formData.isAdditionalExam) && "(Optional)"}</Label>
                    <Input 
                        type="number" 
                        value={formData.year} 
            ...
            <CardContent className="space-y-4">
                              ...
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent>
                                  ...
                                  <DropdownMenuItem onClick={() => addScenarioContentBlock("pseudocode", blockIndex)}>
                                    <FileText className="h-4 w-4 mr-2" /> Pseudocode
                                  </DropdownMenuItem>
                            ...
                            <div className="p-3 bg-neutral-50 dark:bg-neutral-900 rounded-lg border">
                                <div className="flex justify-between items-center mb-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {block.type === "pseudocode" && <FileText className="h-4 w-4 text-neutral-500" />}
                                        ...
                                        <span className="text-sm font-medium capitalize">{block.type === "code-table" ? "Code Table" : block.type === "data-table" ? "Data Table" : block.type === "database-schema" ? "DB Schema" : block.type === "row-layout" ? "Side-by-Side Layout" : block.type}</span>
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex flex-wrap gap-2 mb-1 items-center">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                />
                                                <Label htmlFor={`border-${block.id}`} className="text-xs cursor-pointer">Border</Label>
                                            </div>
                                            {block.hasBorder && (
                                                <Select
                                            ...
                                            <tbody>
                                                {block.pseudocodeLines?.map((line, lineIndex) => (
                                                    <tr key={line.id} className="group">
                                                        <td className="pr-2 w-24 align-top">
                                                            <Input
                                                        ...
                                                        <td className="pl-1 w-8 align-top">
                                                            <Button
                                                                variant="ghost"
                                                                size="sm"
                                                ...
                                                <RowLayoutItem key={childBlock.id}>
                                                    <div className="p-2 bg-white dark:bg-neutral-800 rounded border text-sm">
                                                        <div className="flex items-center gap-1 mb-1 text-xs text-neutral-500">
                                                            ...
                                                            {(childBlock.type === "data-table" || childBlock.type === "code-table") && <Table className="h-3 w-3" />}
                                                            {childBlock.type === "database-schema" && <Database className="h-3 w-3" />}
                                                            <span className="capitalize">{childBlock.type === "data-table" ? "Data Table" : childBlock.type === "code-table" ? "Code Table" : childBlock.type}</span>
                                                        </div>
                                                            ...
                                                            <div className="border border-neutral-300 dark:border-neutral-700 rounded-lg overflow-hidden">
                                                                {childBlock.codeSections.map((section, sIdx) => (
                                                                    <div key={section.id || sIdx}>
                                                                        <div className="bg-neutral-200 dark:bg-neutral-700 px-2 py-1 font-semibold text-xs border-b border-neutral-300 dark:border-neutral-600">{section.label}</div>
                                                                        <pre className="bg-neutral-900 text-neutral-100 p-2 text-xs font-mono overflow-x-auto">{section.code}</pre>
                                                                    </div>
                                                                ))}
                                                            </div>
                                                        )}
                ...
                <div className="flex flex-wrap gap-2 pt-2 border-t">
                    ...
                    <Button
                        ...
                        type="button"
                    >
                        <FileText className="h-3 w-3 mr-1" /> Add Pseudocode
                    </Button>
                    <Button
                        variant="outline"
                        size="sm"
            ...
            {formData.subQuestions.map((subQ, index) => (
                <AccordionItem key={subQ.id || index} value={`question-${index}`} className="border rounded-lg bg-white dark:bg-neutral-900">
                    ...
                    <AccordionContent className="px-4 pb-4">
                        ...
                        <Collapsible defaultOpen={subQ.inputStyle !== "text" && subQ.inputStyle !== "info-only"}>
                        ...
                        {/* Navigation Structure Configuration (N5) */}
                        {subQ.inputStyle === "nav-structure" && (
                          <div className="space-y-4 p-4 bg-neutral-50 dark:bg-neutral-900 rounded-lg border">
                            <div>
                              <Label className="text-sm font-semibold">Starting Diagram (Optional)</Label>
                              <p className="text-xs text-neutral-500 mt-1">
                                Draw a starting navigation structure that students will complete. Leave empty if students should create from scratch.
                              </p>
                            </div>
                            ...
                            <div className="border-t pt-4 space-y-4">
                              <div className="flex justify-between items-start">
                                <div>
                                  <Label className="text-sm font-semibold text-green-700 dark:text-green-400">Example Answer (For AI Grading)</Label>
                                  <p className="text-xs text-neutral-500 mt-1">
                                    Draw the expected answer. The AI will use this to grade student submissions more accurately.
                                  </p>
                                </div>
                                {subQ.inputConfig?.navExampleData && (
                                  <Button
                                    size="sm"
                                    variant="outline"
                              ...
                              <SubQuestionDiagramFieldEditor
                                ...
                                inputConfig={subQ.inputConfig}
                                updateSubQuestion={updateSubQuestion}
                                dataField="navExampleData"
                                drawingField="navExampleCanvas"
                        ...
                        {/* Navigation Structure Configuration (Higher) */}
                        ...
                        <div className="space-y-3">
                                          ...
                                          <div className="p-3 bg-neutral-50 dark:bg-neutral-900 rounded-lg border">
                                            <div className="flex justify-between items-center mb-2">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {block.type === "database-schema" && <Database className="h-4 w-4 text-neutral-500" />}
                                                    {block.type === "row-layout" && <Layers className="h-4 w-4 text-blue-500" />}
                                                    <Label className="text-sm font-medium capitalize">{block.type === "code-table" ? "Code Table" : block.type === "data-table" ? "Data Table" : block.type === "database-schema" ? "DB Schema" : block.type === "row-layout" ? "Side-by-Side Layout" : block.type}</Label>
                                                </div>
                                                <div className="flex items-center gap-1">
                                                    <Button
                                                        variant="ghost"
                            ...
                            <div className="flex flex-wrap gap-2 pt-2 border-t border-dashed border-neutral-200 dark:border-neutral-700">
                                ...
                                <Button
                                    ...
                                    type="button"
                                >
                                    <FileText className="h-3 w-3 mr-1" /> Pseudocode
                                </Button>
                            </div>
                        </div>
                        ...
                        <div className="pt-4 border-t border-neutral-200 dark:border-neutral-700 space-y-4">
                            ...
                            {(subQ.inputStyle === "webpage-wireframe" || subQ.inputStyle === "form-wireframe") && (
                            <div className="space-y-3 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                              <div className="flex justify-between items-center flex-wrap gap-2">
                                <Label className="text-sm font-semibold">Example Drawing (for AI grading)</Label>
                                {subQ.inputConfig?.wireframeExampleData && (
                                  <Button 
                                    size="sm" 
                                    variant="outline"
                                  ...
                                  >
                                    <Trash2 className="w-3 h-3 mr-1" /> Clear Example
                                  </Button>
                                )}
                              </div>
                              <p className="text-xs text-neutral-500">Draw an example of what the student's answer should look like. The AI will compare student submissions against this.</p>
                              <SubQuestionDiagramFieldEditor
                                subIndex={index}
                                inputConfig={subQ.inputConfig}
                                updateSubQuestion={updateSubQuestion}
                                dataField="wireframeExampleData"
                                drawingField="wireframeExampleCanvas"
                                            ...
                                            <Accordion type="single" collapsible className="bg-neutral-50 dark:bg-neutral-900 rounded-lg border">
                                                <AccordionItem value={`part-${partIndex}`} className="border-none">
                                                    ...
                                                    <AccordionContent className="px-4 pb-4 space-y-3">
                                                        ...
                                                        <div className="space-y-2">
                                                                                ...
                                                                                <div className="space-y-2">
                                                                                    <div className="flex flex-wrap gap-2 items-center">
                                                                                        ...
                                                                                        <div className="flex items-center gap-1">
                                                                                            ...
                                                                                            />
                                                                                            <Label htmlFor={`subpart-border-${block.id}`} className="text-xs cursor-pointer">Border</Label>
                                                                                        </div>
                                                                                        {block.hasBorder && (
                                                                                            <Select
                                                                                                value={block.borderWidth || "md"}
                                                                                ...
                                                                                <div className="space-y-2">
                                                                                    <table className="w-full font-mono text-xs border-collapse">
                                                                                        <tbody>
                                                                                            {block.pseudocodeLines?.map((line, lineIndex) => (
                                                                                                <tr key={line.id} className="group">
                                                                                                    <td className="pr-2 w-20 align-top">
                                                                                                        <Input
                                                                                                            value={line.lineLabel}
                                                                                                    ...
                                                                                                    <td className="pl-1 w-6 align-top">
                                                                                                        <Button
                                                                                                            variant="ghost"
                                                                                                            size="sm"
                                                                                    ...
                                                                                    >
                                                                                        <Plus className="h-2.5 w-2.5 mr-1" /> Add Line
                                                                                    </Button>
                                                                                </div>
                                                                            )}
                                                            ...
                                                            <div className="flex flex-wrap gap-1 pt-2 border-t border-dashed border-neutral-200 dark:border-neutral-700">
                                                                ...
                                                                <Button
                                                                    ...
                                                                    className="h-6 text-xs"
                                                                >
                                                                    <FileText className="h-2.5 w-2.5 mr-1" /> Pseudocode
                                                                </Button>
                                                                <Button
                                                                    variant="outline"
                                                                    size="sm"
                                                        ...
                                                        {/* Navigation Structure Configuration for Sub-Parts */}
                                                        {part.inputStyle === "nav-structure" && (
                                                          <div className="space-y-4 p-3 bg-neutral-100 dark:bg-neutral-800 rounded-lg">
                                                            ...
                                                            <div className="border-t pt-4 space-y-4">
                                                              <div className="flex justify-between items-start">
                                                                <div>
                                                                  <Label className="text-xs font-semibold text-green-700 dark:text-green-400">Example Answer (For AI Grading)</Label>
                                                                  <p className="text-xs text-neutral-500 mt-1">
                                                                    Draw the expected answer. The AI will use this to grade student submissions.
                                                                  </p>
                                                                </div>
                                                                {part.inputConfig?.navExampleData && (
                                                                  <Button
                                                                    size="sm"
                                                                    variant="outline"
                                                                    className="text-red-500 hover:text-red-600 shrink-0"
                                                            ...
                                                            {part.drawingBackgroundUrl && (
                                                              <div className="flex items-center gap-2">
                                                                ...
                                                                <Button 
                                                                  ...
                                                                  variant="ghost" 
                                                                  className="text-red-500"
                                                                  onClick={() => updateSubPart(index, partIndex, "drawingBackgroundUrl", undefined)}
                                                                >
                                                                  <Trash2 className="h-4 w-4" />
                                                                </Button>
                                                              </div>
                                                        ...
                                                        {(part.inputStyle === "webpage-wireframe" || part.inputStyle === "form-wireframe") && (
                                                          <div className="space-y-3 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                                                            <div className="flex justify-between items-center flex-wrap gap-2">
                                                              <Label className="text-xs font-semibold">Example Drawing (for AI grading)</Label>
                                                              {part.inputConfig?.wireframeExampleData && (
                                                                <Button 
                                                                  size="sm" 
                                                                  variant="outline"
                                                                  className="text-red-500 hover:text-red-600"
                                                            ...
                                                            <p className="text-xs text-neutral-500">Draw an example for the AI to compare against student answers.</p>
                                                            <SubPartDiagramFieldEditor
                                                              subIndex={index}
                                                              partIndex={partIndex}
                                                              inputConfig={part.inputConfig}
                ...
                {formData.scenario.contentBlocks && formData.scenario.contentBlocks.length > 0 ? (
                  <div className="space-y-4">
                    {formData.scenario.contentBlocks.map((block: ContentBlock) => (
                      <div key={block.id}>
                        {block.type === "text" && (
                          <div className={`text-neutral-700 dark:text-neutral-300 leading-relaxed whitespace-pre-wrap ${block.textAlign === "center" ? "text-center" : block.textAlign === "right" ? "text-right" : "text-left"}`}>
                            {block.content}
                          </div>
                        )}
                          ...
                          <RowLayout>
                            {block.children.map((child: ContentBlock, childIdx: number) => (
                              <RowLayoutItem key={child.id || childIdx}>
                                ...
                                {child.type === "pseudocode" && child.pseudocodeLines && (
                                  <table className="font-mono text-sm">
                                    <tbody>
                                      {child.pseudocodeLines.map((line, idx) => (
                                        <tr key={line.id || idx}>
                                          <td className="pr-4 text-neutral-500 dark:text-neutral-400 align-top whitespace-nowrap">{line.lineLabel}</td>
                                          <td className="text-neutral-900 dark:text-neutral-100 whitespace-pre">{line.content}</td>
                                        </tr>
                                ...
                                {child.type === "code-table" && child.codeSections && (
                                  <div className="border border-neutral-300 dark:border-neutral-700 rounded-lg overflow-hidden">
                                    {child.codeSections.map((section, sIdx) => (
                                      <div key={section.id || sIdx}>
                                ...
                                {child.type === "data-table" && child.dataTable && (
                                  <ResponsiveDataTable dataTable={child.dataTable} />
                                )}
                                {child.type === "database-schema" && child.databaseSchema && (
                                  <DatabaseSchemaDisplay schema={child.databaseSchema} />
                                )}
                              </RowLayoutItem>
                            ))}
                          </RowLayout>
                                ...
                                block.hasBorder ? (
                                  <div className="flex justify-center">
                                    <div className={cn(
                                      ...
                                      <RichTextDisplay content={block.content} className="text-lg font-medium text-neutral-900 dark:text-white leading-relaxed" />
                                    </div>
                                  </div>
                                ) : (
                                  <RichTextDisplay content={block.content} className={cn(
                              ...
                              {block.type === "row-layout" && block.children && (
                                <RowLayout>
                                  {block.children.map((child: ContentBlock, childIdx: number) => (
                                    <RowLayoutItem key={child.id || childIdx}>
                                      {child.type === "text" && (
                                        <RichTextDisplay content={child.content} className="text-neutral-900 dark:text-white leading-relaxed" />
                                      )}
                                      {child.type === "image" && child.content && (
                                        <div className="flex flex-col items-center">
                                          {child.caption && <p className="text-sm text-neutral-500 dark:text-neutral-400 text-center mb-2">{child.caption}</p>}
                                            ...
                                            block.hasBorder ? (
                                              <div className="flex justify-center">
                                                <div className={cn(
                                                  ...
                                                  <RichTextDisplay content={block.content} className="text-base text-neutral-900 dark:text-white leading-relaxed" />
                                                </div>
                                              </div>
                                            ) : (
                                              <RichTextDisplay content={block.content} className={cn(
