...
The Python Editor (`HTML/Tools/PythonEditor.html`) and HTML/CSS Editor (`HTML/Tools/HTMLEditor.html`) tool pages now show a personal project dashboard when no `?project=<id>` query parameter is present. Students can create as many named projects as they like; each project is a single primary file (`main.py` or `index.html`) saved to `localStorage` via the small `JavaScript/codeProjects.js` helper (`bhs_code_projects_python` / `bhs_code_projects_html` keys). Opening a project navigates to `?project=<id>`, which loads the saved code into a runner element built dynamically and initialised via the new `window.CodeRunner.initPy` / `window.CodeRunner.initHtml` exports added to `JavaScript/codeRunner.js`. The editor auto-saves on `input` (400 ms debounce). The tool header shows a "← Projects" back link and the project name (click to rename), giving every tool a way back to its dashboard. The Database Sandbox tool already provides the same back-to-dashboard affordance via the existing ribbon Home button (`allDatabasesLink="/"`). Dashboard styles live in `CSS/codeProjects.css`.
