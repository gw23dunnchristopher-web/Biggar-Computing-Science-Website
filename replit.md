...
- **March 30, 2026**: Moved all teacher tools (login, account settings) out of the sidebars and into the Teacher Dashboard (`/tools/sandbox-builder.html`). Sidebars no longer show any teacher UI. Dashboard header now has Account Settings modal (update email/password) and Sign Out button. Sandbox Builder is now one tool within the dashboard; future tools will be added alongside it. Removed all teacher-bar/teacher-modal CSS from `CSS/sidebarStyles.css`.
