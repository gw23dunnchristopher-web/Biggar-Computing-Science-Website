...
### BHS Classwork (`/classwork/`)
A new SPA (`classwork-client/`, React + Vite + Tailwind v4, built with `npm run build:classwork` → `public/classwork/`) for AI-marked classwork. Five separate course worlds: S1, S2, S3, N5, Higher (chosen on Home page). Hierarchy: Units → Lessons → Questions → Submissions, all stored in `bhs_classwork_units`, `bhs_classwork_lessons`, `bhs_classwork_questions`, `bhs_classwork_submissions` (raw-SQL `CREATE TABLE IF NOT EXISTS` on first call to `ensureClassworkSchema()` in `server/classwork-storage.ts`). Each table has a `course` column constrained to one of the five course keys. Question types currently scaffolded: `short`, `long`, `code`, `multiple_choice`, `screenshot`, `scratch_link`, `makecode_link`, `google_sites_link`, `project` (file uploads, AI marking and analytics still to ship in later phases). Routes are registered by `server/classwork-routes.ts` under `/api/classwork/*`. Auth reuses the existing flows: students sign in with the same Bearer `studentToken` shared with the Higher/N5 revision apps and the site-wide login pill; teachers sign in with their dashboard email + password (`/api/teacher-auth`) and the resulting token is sent in the `x-teacher-password` header (the same `requireTeacher` predicate used everywhere else). The site-wide pill (`JavaScript/siteAuth.js`) gains a "BHS Classwork" link beside the username pill whenever any user is signed in.
