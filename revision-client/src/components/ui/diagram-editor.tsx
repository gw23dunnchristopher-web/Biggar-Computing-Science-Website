...
export interface DiagramItem {
  ...
  lineType?: "1:1" | "1:M" | "M:M"; // ERD cardinality for lines/crowfoot
...
interface DiagramEditorProps {
  ...
  mode?: "flowchart" | "database" | "wireframe" | "general" | "erd-annotation" | "nav-structure" | "nav-structure-higher" | "structure-dataflow" | "form-wireframe" | "structure-diagram" | "entity-occurrence"; // Controls which tools are shown
...
function DiagramEditorComponent({ initialData, initialDrawing, onChange, disabled, backgroundUrl, mode = "general", baseDiagram, showFunctionNumbers = false, allowBaseItemDeletion = false }: DiagramEditorProps) {
      ...
      try {
        ...
        if (initialData) {
          ...
          const mergedBase = baseItems.map(bi => {
            ...
            if (!studentVersion) return bi;
            const preserved: Partial<DiagramItem> = { marking: studentVersion.marking };
            if (studentVersion.relationshipLabel !== undefined) preserved.relationshipLabel = studentVersion.relationshipLabel;
            if (studentVersion.lineType !== undefined) preserved.lineType = studentVersion.lineType;
            if (studentVersion.entityName !== undefined) preserved.entityName = studentVersion.entityName;
            if (studentVersion.attributes !== undefined) preserved.attributes = studentVersion.attributes;
            if (studentVersion.content !== undefined) preserved.content = studentVersion.content;
            return { ...bi, ...preserved };
          });
  ...
  // Dynamic height based on background image
  const [containerHeight, setContainerHeight] = useState(400);
  
  // Load background image and calculate height
  useEffect(() => {
    if (!backgroundUrl) {
    ...
    img.onload = () => {
      // Calculate height to maintain aspect ratio within container width
      const containerWidth = containerRef.current?.clientWidth || 800;
      const aspectRatio = img.height / img.width;
      const calculatedHeight = Math.max(400, Math.min(800, containerWidth * aspectRatio));
  ...
  const addItem = (type: DiagramItem["type"], x: number, y: number, width?: number, height?: number, x2?: number, y2?: number, connectedTo1?: string, connectedTo2?: string, anchor1Side?: 'top' | 'bottom' | 'left' | 'right', anchor2Side?: 'top' | 'bottom' | 'left' | 'right') => {
    ...
    const newItem: DiagramItem = {
      ...
      lineType: isLineType ? pendingLineType : undefined,
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
        // Use editingItemId to determine if we're in text edit mode
        // This way, single-click selects (copy/paste works), double-click enables text editing
  ...
  return (
    <div className="flex flex-col gap-2">
            ...
            {mode === "structure-dataflow" ? (
              ...
              <>
                <Button
                    ...
                    size="sm"
                    onClick={() => setTool("box")}
                    title="Entity (Rectangle)"
                    className={cn("px-2 sm:px-3", tool === "box" && "bg-blue-500 text-white hover:bg-blue-600")}
                >
                    <Square className="w-4 h-4 sm:mr-1" /> <span className="hidden sm:inline">Entity</span>
                </Button>
                <Button
                    variant={tool === "ellipse" ? "secondary" : "ghost"}
                    size="sm"
                    onClick={() => setTool("ellipse")}
                    title="Attribute (Ellipse)"
                    className={cn("px-2 sm:px-3", tool === "ellipse" && "bg-blue-500 text-white hover:bg-blue-600")}
                >
                    <Circle className="w-4 h-4 sm:mr-1" /> <span className="hidden sm:inline">Attribute</span>
                </Button>
                <div className="w-px h-6 bg-neutral-300 dark:bg-neutral-600 mx-1" />
                <Button
                    variant={(tool === "line" || tool === "crowfoot") && pendingLineType === "1:1" ? "secondary" : "ghost"}
                    size="sm"
                    onClick={() => { setTool("line"); setPendingLineType("1:1"); }}
                    title="1:1 Line (no forks)"
                    className={cn("px-2 sm:px-3", (tool === "line" || tool === "crowfoot") && pendingLineType === "1:1" && "bg-blue-500 text-white hover:bg-blue-600")}
                >
                    <Minus className="w-4 h-4 sm:mr-1" /> <span className="hidden sm:inline">1:1</span>
                </Button>
                <Button
                    variant={(tool === "line" || tool === "crowfoot") && pendingLineType === "1:M" ? "secondary" : "ghost"}
                    size="sm"
                    onClick={() => { setTool("crowfoot"); setPendingLineType("1:M"); }}
                    title="1:M Line (fork at one end)"
                    className={cn("px-2 sm:px-3", (tool === "line" || tool === "crowfoot") && pendingLineType === "1:M" && "bg-blue-500 text-white hover:bg-blue-600")}
                >
                    <Route className="w-4 h-4 sm:mr-1" /> <span className="hidden sm:inline">1:M</span>
                </Button>
                <Button
                    variant={(tool === "line" || tool === "crowfoot") && pendingLineType === "M:M" ? "secondary" : "ghost"}
                    size="sm"
                    onClick={() => { setTool("crowfoot"); setPendingLineType("M:M"); }}
                    title="M:M Line (forks at both ends)"
                    className={cn("px-2 sm:px-3", (tool === "line" || tool === "crowfoot") && pendingLineType === "M:M" && "bg-purple-500 text-white hover:bg-purple-600")}
                >
                    <Route className="w-4 h-4 sm:mr-1 scale-x-[-1]" /> <span className="hidden sm:inline">M:M</span>
                </Button>
              </>
            ) : (
              <>
      ...
      <div
        ...
        className={cn(
          `relative w-full bg-white border-2 border-neutral-200 dark:border-neutral-700 rounded-lg overflow-hidden select-none diagram-bg focus:outline-none focus:ring-2 focus:ring-red-500`,
      ...
      >
        {backgroundUrl && (
             <img 
                src={backgroundUrl} 
                className="absolute inset-0 w-full h-full object-contain opacity-80 pointer-events-none z-0 mix-blend-multiply dark:mix-blend-normal" 
                alt="Background"
             />
        )}
        ...
        {items.filter(item => item.type === "line" || item.type === "crowfoot" || item.type === "dataflow-arrow").map((item) => {
          ...
          // For crow's foot lines, ERD lines, and nav-structure lines, clip to shape edges
          if (item.type === "crowfoot" || ((item.type === "line") && (mode === "erd-annotation" || mode === "database")) || mode === "nav-structure" || mode === "nav-structure-higher" || mode === "structure-diagram" || mode === "entity-occurrence") {
          ...
          return (
            <svg key={item.id} className={cn("absolute inset-0 w-full h-full pointer-events-none", selectedId === item.id ? "z-[100]" : mode === "entity-occurrence" ? "z-[50]" : "z-[5]")} style={{ overflow: 'visible' }}>
                  ...
                  {selectedId === item.id && !disabled ? (
                    <>
                      ...
                      <foreignObject
                        ...
                        style={{ overflow: 'visible' }}
                      >
                        <div className="flex gap-0.5 bg-white border border-neutral-300 rounded shadow-lg p-0.5">
                          {(["1:1", "1:M", "M:M"] as const).map(lt => (
                            <button
                              key={lt}
                              onClick={(e) => {
                                e.stopPropagation();
                                if (lt === "1:1") {
                                  updateItem(item.id, { type: "line" as any, lineType: "1:1" });
                                } else {
                                  updateItem(item.id, { type: "crowfoot" as any, lineType: lt });
                                }
                              }}
                              className={cn(
                                "px-2.5 py-0.5 text-xs rounded font-medium",
                                (item.lineType || (item.type === "crowfoot" ? "1:M" : "1:1")) === lt
              ...
              {/* Crow's foot notation - renders forks based on lineType */}
              {item.type === "crowfoot" && (() => {
                const sColor = selectedId === item.id ? "#ef4444" : "#262626";
                const sW = selectedId === item.id ? 3 : 2;
                const footLength = 12;
                const prongSpread = 8;
                const perpXDir = Math.cos(angle + Math.PI / 2);
                const perpYDir = Math.sin(angle + Math.PI / 2);
                const lt = item.lineType || "1:M";
                const elements: React.ReactNode[] = [];
                
                if (lt === "1:M" || lt === "M:M") {
                  const convergeX = x2 - Math.cos(angle) * footLength;
                  const convergeY = y2 - Math.sin(angle) * footLength;
                  const topEdgeX = x2 + perpXDir * prongSpread;
                  const topEdgeY = y2 + perpYDir * prongSpread;
                  const bottomEdgeX = x2 - perpXDir * prongSpread;
                  const bottomEdgeY = y2 - perpYDir * prongSpread;
                  elements.push(
                    <line key="end-top" x1={convergeX} y1={convergeY} x2={topEdgeX} y2={topEdgeY} stroke={sColor} strokeWidth={sW} />,
                    <line key="end-bot" x1={convergeX} y1={convergeY} x2={bottomEdgeX} y2={bottomEdgeY} stroke={sColor} strokeWidth={sW} />
                  );
                }
                
                if (lt === "M:M") {
                  const angleReverse = angle + Math.PI;
                  const convergeX = x1 - Math.cos(angleReverse) * footLength;
                  const convergeY = y1 - Math.sin(angleReverse) * footLength;
                  const topEdgeX = x1 + perpXDir * prongSpread;
                  const topEdgeY = y1 + perpYDir * prongSpread;
                  const bottomEdgeX = x1 - perpXDir * prongSpread;
                  const bottomEdgeY = y1 - perpYDir * prongSpread;
                  elements.push(
                    <line key="start-top" x1={convergeX} y1={convergeY} x2={topEdgeX} y2={topEdgeY} stroke={sColor} strokeWidth={sW} />,
                    <line key="start-bot" x1={convergeX} y1={convergeY} x2={bottomEdgeX} y2={bottomEdgeY} stroke={sColor} strokeWidth={sW} />
                  );
                }
        ...
        {items
          ...
          <div
            ...
            style={{
              left: item.x,
              top: item.y,
              width: (item.type !== "text" && item.type !== "bullet-text" && item.type !== "numbered-text" && item.type !== "link-text") ? (item.width || (item.type === "circle" ? 40 : 120)) : "auto",
              height: (item.type !== "text" && item.type !== "bullet-text" && item.type !== "numbered-text" && item.type !== "link-text") ? (item.height || (item.type === "circle" ? 40 : 60)) : "auto",
            }}
            className={cn(
          ...
          >
             ...
             {!item.type.startsWith("ui-") && item.type !== "text" && item.type !== "bullet-text" && item.type !== "numbered-text" && item.type !== "link-text" && item.type !== "line" && item.type !== "crowfoot" && (
            ...
            {(item.type === "text" || item.type === "bullet-text" || item.type === "numbered-text" || item.type === "link-text") ? (
              <div className={cn(
                ...
                {selectedId === item.id && !disabled && mode !== "erd-annotation" && (
                  <div className="absolute -top-8 left-0 flex gap-1 bg-white dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-600 rounded shadow-lg p-1 z-50">
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); updateItem(item.id, { isUnderline: !item.isUnderline }); }}
                      className={cn(
                  ...
                  {(item.type === "bullet-text" || item.type === "numbered-text") ? (
                    ...
                    <>
                      ...
                      <textarea
                        ...
                        spellCheck={false}
                        className={cn(
                          "bg-transparent border-none focus:ring-0 p-1 min-w-[80px] resize-none overflow-hidden",
                          item.parentEntityId ? "text-black" : "text-neutral-900",
                          item.isBold && "font-bold",
                          !item.isBold && "font-medium",
                        ...
                        placeholder="Enter variables (one per line)"
                      />
              ...
              <>
               ...
               <div className="w-full h-full relative flex items-center justify-center">
                 ...
                 <input
                    ...
                    placeholder="Image"
                    onChange={(e) => updateItem(item.id, { content: e.target.value })}
                    className="bg-white/80 border-none focus:ring-0 text-center w-auto px-2 outline-none cursor-move placeholder:text-neutral-400 text-xs z-10"
              ...
              <div className="w-full h-full flex flex-col bg-white border-2 border-neutral-800 rounded overflow-hidden">
               ...
               <div className={cn("w-full h-full flex items-center justify-center text-sm text-neutral-900 font-semibold z-10 relative overflow-hidden", getTextClasses(item.type))}>
                  {/* Edit mode: contentEditable when double-clicked */}
                  {editingItemId === item.id && !disabled ? (
                    <div 
                      ...
                      className={cn(
                        "w-full h-full flex items-center justify-center bg-transparent outline-none text-center px-1 whitespace-pre-wrap border-2 border-blue-400",
                        item.marking === "primary" && "underline decoration-2 decoration-red-600"
                      )}
                      style={{ 
                        fontSize: '14px',
