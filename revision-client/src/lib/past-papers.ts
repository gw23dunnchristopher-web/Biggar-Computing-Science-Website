...
export type ContentBlockType = "text" | "image" | "code" | "code-table" | "data-table" | "database-schema" | "row-layout" | "pseudocode";
...
// Pseudocode line for structured code display
export interface PseudocodeLine {
  id: string;
  lineNumber: number;
  content: string;
  indent?: number; // Number of indent levels (each level = 4 spaces)
}
...
export interface ContentBlock {
  ...
  hasBorder?: boolean; // Whether to display a border around the block
  borderWidth?: 1 | 2 | 3 | 4; // Border width in pixels
  ...
  pseudocodeLines?: PseudocodeLine[]; // For pseudocode blocks: structured lines with line numbers
}
...
export interface SubQuestion {
  ...
  markingGuidanceData?: {
    rows?: { part: string; expectedResponse: string; additionalGuidance: string; marks: number }[];
  ...
  inputConfig?: {
    ...
    // Navigation structure (Higher): solution diagram for AI grading
...
export interface Question {
  ...
  isQuizOnly?: boolean; // True if this question only appears in custom quizzes
  isAdditionalExam?: boolean; // True if this is a teacher-created additional exam (no year)
  additionalExamId?: string | null;
  scenario?: {
...
export const PAST_PAPERS: Question[] = [
  // Higher Level questions will be added through the Teacher Dashboard
];
