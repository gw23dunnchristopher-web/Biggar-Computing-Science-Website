...
export default function ClassManager() {
  ...
  const fetchStudents = useCallback(async (classId: string) => {
    try {
      const res = await fetch(`/api/classes/${classId}/students`, { headers: getAuthHeaders() });
      if (res.ok) {
  ...
  const handleDeleteClass = async () => {
    ...
    try {
      const res = await fetch(`/api/classes/${deleteClassTarget.id}`, {
        method: "DELETE",
        headers: getAuthHeaders(),
      });
      if (res.ok) {
  ...
  const handleAddStudent = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStudentCount(1);
        setCredentialModal({
          credentials: created.map((s: any) => ({ username: s.username, password: s.plainPassword })),
          title: created.length === 1 ? "New Student Created" : `${created.length} Students Created`,
        });
  ...
  const handleDeleteStudent = async () => {
    ...
    try {
      const res = await fetch(`/api/students/${deleteStudentTarget.id}`, {
        method: "DELETE",
        headers: getAuthHeaders(),
      });
      if (res.ok) {
  ...
  const handleRenameStudent = async () => {
    ...
    try {
      const res = await fetch(`/api/students/${renameTarget.id}/username`, {
        method: "PATCH",
        headers: getAuthHeaders(),
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  {students.length > 0 && (
                    <Button
                      variant="outline"
                      size="sm"
                      data-testid="button-download-usernames"
                      onClick={() => {
      ...
      <Dialog open={isAddStudentOpen} onOpenChange={setIsAddStudentOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Choose how many students to add. Each will get a friendly username (e.g. brave-fox-42) and password. You can rename students afterwards.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddStudent} data-testid="button-confirm-add-student">
      ...
      <Dialog open={!!renameTarget} onOpenChange={(open) => { if (!open) setRenameTarget(null); }}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Change the username for this student. They will need to use the new username to log in. Only lowercase letters, numbers, hyphens, and underscores are allowed.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
