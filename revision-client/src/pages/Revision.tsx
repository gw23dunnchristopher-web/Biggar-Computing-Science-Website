...
import { ArrowLeft, CheckCircle2, XCircle, ChevronRight, ChevronDown, RefreshCw, Wand2, List, Code2, FileEdit, Upload, X, FileText, Loader2, Download } from "lucide-react";
...
export default function Revision() {
  ...
  const handleAutoMark = async () => {
    ...
    try {
      ...
      const prepareStudentAnswer = (sub: SubQuestion, inputs: Record<string, string>): string => {
          ...
          try {
            ...
            const shapeDescriptions = items
              ...
              .map(i => {
                ...
                if (["ui-input", "ui-output", "ui-dropdown"].includes(i.type)) {
                  ...
                  return '[' + shapeType + ' ' + posStr + labelText + ': "' + (i.content || "(no label)") + '"' + baseMarker + ']';
                }
                return '[' + shapeType + ' ' + posStr + ': "' + (i.content || "(no label)") + '"' + baseMarker + ']';
              });
            ...
            const textDescriptions = items
              ...
              .map(i => {
                ...
                if (i.textAlign && i.textAlign !== "left") formatting.push(`aligned-${i.textAlign}`);
                const formatStr = formatting.length > 0 ? ` (${formatting.join(", ")})` : "";
                const posStr = `at approx (${Math.round(i.x)}, ${Math.round(i.y)}), size: ${Math.round(i.width || 100)}x${Math.round(i.height || 30)}`;
                const baseMarker = i.isBaseItem ? " [base]" : "";
                
                if (i.type === "bullet-text" && i.content) {
                  const bulletPoints = i.content.split("\n").filter(line => line.trim());
                  return '[BULLET_LIST ' + posStr + ': ' + bulletPoints.length + ' bullet points: ' + bulletPoints.map((p, idx) => (idx + 1) + '. "' + p + '"').join(', ') + formatStr + baseMarker + ']';
                }
                
                if (i.type === "numbered-text" && i.content) {
                  const numberedItems = i.content.split("\n").filter(line => line.trim());
                  return '[NUMBERED_LIST ' + posStr + ': ' + numberedItems.length + ' numbered items: ' + numberedItems.map((p, idx) => (idx + 1) + '. "' + p + '"').join(', ') + formatStr + baseMarker + ']';
                }
                
                return '[TEXT ' + posStr + ': "' + (i.content || "(no label)") + '"' + formatStr + baseMarker + ']';
              });
        ...
        // Handle Higher level navigation structure - describe hierarchical structure with nav highlight areas
        ...
        console.log("DEBUG form-wireframe check:", { inputStyle: sub.inputStyle, hasDrawing: !!inputs["drawing"], inputs });
        if (sub.inputStyle === "form-wireframe" && inputs["drawing"]) {
          try {
            const items = JSON.parse(inputs["drawing"]) as DiagramItem[];
            console.log("DEBUG form-wireframe items:", items);
            ...
            console.log("DEBUG form-wireframe result:", result);
            return result;
          } catch (e) {
            return "";
          }
      ...
      const gradingPromises = allSubQuestions
        ...
        .map(async (sub) => {
          ...
          console.log("DEBUG grading studentAnswer:", { subId: sub.id, inputStyle: sub.inputStyle, studentAnswer, trimmed: studentAnswer.trim() });
          
          totalMaxMarks += sub.maxMarks;

          if (!studentAnswer.trim()) {
            console.log("DEBUG: Empty answer, skipping grading for", sub.id);
          ...
          try {
            ...
            if (sub.requiresStudentCode) {
              for (const otherSub of allSubQuestions) {
                if (otherSub.id === sub.id) continue;
                const otherInputs = userInputs[otherSub.id] || {};
                if (otherInputs["uploaded_files"]) {
                  try {
                    const files = JSON.parse(otherInputs["uploaded_files"]) as { url: string; name: string }[];
            ...
            console.log("DEBUG: Grade API response status for", sub.id, response.status, response.ok);
            if (response.ok) {
              const result = await response.json();
              console.log("DEBUG: Grade API result for", sub.id, result);
              return {
                subId: sub.id,
                marks: result.marks,
              ...
              console.log("DEBUG: Grade API failed for", sub.id, "falling back to keyword matching");
              const marks = calculateMarks(inputs, sub);
              return { subId: sub.id, marks, feedback: null };
            }
          } catch (error: any) {
            console.error("DEBUG: Grade API error for", sub.id);
  ...
  const renderFileUpload = (subQ: SubQuestion) => {
    ...
    const handleUpload = async (files: FileList) => {
      ...
      handleInputChange(subQ.id, "uploaded_files", JSON.stringify(newFiles));
    ...
    return (
      <div className="mt-3 border border-dashed border-neutral-300 dark:border-neutral-700 rounded-lg p-3">
        <div className="flex items-center justify-between mb-2">
          ...
          <label className="cursor-pointer">
            <input
              ...
              multiple
              className="hidden"
              disabled={showResults || fileUploading === subQ.id}
              onChange={(e) => {
        ...
        <p className="text-xs text-neutral-500 mb-2">Upload screenshots, code files, or other evidence (drag & drop, click, or paste).</p>
  ...
  const makePasteHandler = (subQ: SubQuestion) => (e: React.ClipboardEvent) => {
    if (showResults || !subQ.allowFileUpload) return;
      ...
      if (item.type.startsWith("image/")) {
        ...
        fetch("/api/upload-student-file", { method: "POST", body: formData })
          ...
          .then(result => {
            ...
            handleInputChange(subQ.id, "uploaded_files", JSON.stringify(existing));
          })
          .catch(() => {})
  ...
  const renderInputArea = (subQ: SubQuestion) => {
    ...
    if (subQ.inputStyle === "table" && subQ.inputConfig) {
      if (subQ.inputConfig.grid) {
        ...
        const handleRevisionTableCellChange = (cellKey: string, newValue: string, starterText?: string) => {
          if (starterText && !newValue.startsWith(starterText)) {
            if (newValue.length < starterText.length) {
              handleInputChange(subQ.id, cellKey, starterText);
              return;
            }
          }
          let v = newValue;
          v = v.replace(/^(- |(\* ))/gm, "• ");
          handleInputChange(subQ.id, cellKey, v);
        };

        const handleRevisionTableKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>, cellKey: string, starterText?: string) => {
          ...
          if (e.key === "Enter") {
            ...
            const newVal = before + "\n" + addBullet + after;
            handleInputChange(subQ.id, cellKey, newVal);
            setTimeout(() => {
        ...
        const showGridHeaders = grid.showHeaders !== false;
                          ...
                          <td key={cellIdx} className="px-4 py-2 bg-white dark:bg-neutral-900" style={tdStyle} {...spanProps}>
                            <div className="relative">
                              ...
                              <Textarea
                                ...
                                value={currentVal}
                                onChange={(e) => handleRevisionTableCellChange(cellKey, e.target.value, starter)}
                                onKeyDown={(e) => handleRevisionTableKeyDown(e, cellKey, starter)}
                                disabled={showResults}
    ...
    if (subQ.inputStyle === "drawing") {
      ...
      const questionText = (subQ.questionText + " " + (currentQuestion?.title || "")).toLowerCase();
        ...
        <div className="mt-4">
          <DiagramEditor 
            ...
            onChange={(dataStr, drawingStr) => {
                handleInputChange(subQ.id, "drawing", dataStr);
                handleInputChange(subQ.id, "drawing_canvas", drawingStr);
            }}
            disabled={showResults}
        ...
        <div className="space-y-2 mt-4 h-[500px] border border-neutral-200 dark:border-neutral-800 rounded-lg overflow-hidden bg-white dark:bg-neutral-900">
          <DiagramEditor 
            ...
            onChange={(data, drawing) => {
              handleInputChange(subQ.id, "erd_diagram", data);
              handleInputChange(subQ.id, "erd_drawing", drawing);
            }}
            disabled={showResults}
            backgroundUrl={subQ.drawingBackgroundUrl || subQ.imageUrl}
            mode="erd-annotation"
    ...
    if (subQ.inputStyle === "nav-structure") {
      return (
        <div className="mt-4">
          ...
          />
          <p className="text-xs text-neutral-500 mt-2">
            Draw webpage boxes and connect them with lines. Click a line to add arrows (single arrow = external link, double arrow = internal link).
          </p>
        </div>
      );
    }
    ...
    if (subQ.inputStyle === "nav-structure-higher") {
      return (
        <div className="mt-4">
          ...
          />
          <p className="text-xs text-neutral-500 mt-2">
            Create a hierarchical navigation diagram: Use Page boxes for webpages, Connect lines for parent-child links, and Nav Area to highlight pages in the navigation bar.
          </p>
        </div>
      );
    }
    ...
    if (subQ.inputStyle === "structure-diagram") {
      return (
        <div className="mt-4">
          ...
          />
          <p className="text-xs text-neutral-500 mt-2">
            Create a structure diagram using Process (rectangles) for operations, Decision (diamonds) for conditions, and Loop (ellipses) for iterations. Connect shapes with lines to show program flow.
          </p>
        </div>
      );
    }
      ...
      return (
        <div className="mt-4">
          <DiagramEditor 
            initialData={currentInput["drawing"]}
            initialDrawing={currentInput["drawing_canvas"]}
            baseDiagram={subQ.inputConfig?.baseEntityOccurrenceDiagram}
            onChange={(dataStr, drawingStr) => {
                handleInputChange(subQ.id, "drawing", dataStr);
                handleInputChange(subQ.id, "drawing_canvas", drawingStr);
            }}
          ...
          />
          <p className="text-xs text-neutral-500 mt-2">
            Create an entity-occurrence diagram: Use Entity (tall ovals) to represent entities, add Occurrences inside them, then connect occurrences with lines to show relationships.
          </p>
        </div>
      );
    }
    ...
    if (subQ.inputStyle === "structure-dataflow") {
      return (
        <div className="mt-4">
          ...
          />
          <p className="text-xs text-neutral-500 mt-2">
            Add dataflow arrows to show data passing between functions. Use "Data In" arrows (pointing up) for parameters passed INTO a function, and "Data Out" arrows (pointing down) for values returned FROM a function. Add labels to show variable names.
          </p>
        </div>
      );
    }
        ...
        {!selectedQuestionId ? (
          ...
          <div className="space-y-8">
            ...
            {(() => {
              ...
              const pastPaperQuestions = allQuestions.filter(q => !q.isPractice && !q.isAdditionalExam);
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-neutral-100 text-neutral-800 dark:bg-neutral-800 dark:text-neutral-200">
                    {currentQuestion?.isAdditionalExam ? "Additional Exam" : currentQuestion?.year}
                  </span>
                    ...
                    <div key={subQ.id} className="border-t border-neutral-100 dark:border-neutral-800 pt-6 sm:pt-8 first:border-0 first:pt-0 w-full overflow-hidden">
                      <div className="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-3 mb-4 w-full">
                        ...
                        <div className="flex-1 w-full min-w-0 overflow-hidden">
                              ...
                              <div className="space-y-4 my-4">
                                {subQ.contentBlocks.map((block: ContentBlock) => (
                                  <div key={block.id}>
                                    {block.type === "text" && (
                                      <div className={cn(
                                        "text-base sm:text-lg font-medium text-neutral-900 dark:text-white leading-relaxed whitespace-pre-wrap",
                                        block.textAlign === "center" ? "text-center" : block.textAlign === "right" ? "text-right" : "text-left",
                                        block.hasBorder && "border border-neutral-300 dark:border-neutral-700 p-3 rounded",
                                        block.hasBorder && block.borderWidth === 2 && "border-2",
                                        block.hasBorder && block.borderWidth === 3 && "border-[3px]",
                            ...
                            <div onPaste={makePasteHandler(subQ)}>
                              {renderInputArea(subQ)}
                              {renderFileUpload(subQ)}
                            </div>
                                  ...
                                  <div key={part.id} className="space-y-3">
                                    <div className="flex flex-col sm:flex-row sm:items-start gap-2">
                                      ...
                                      <div className="flex-1">
                                        ...
                                        <div onPaste={makePasteHandler(part)}>
                                          {renderInputArea(part)}
                                          {renderFileUpload(part)}
                                        </div>
                                            ...
                                            {aiFeedback[part.id] && (
                                              <div className="text-xs p-2 bg-blue-50 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-900/50 rounded-md mb-2">
                                                <div className="text-neutral-700 dark:text-neutral-300 space-y-1">
                                                  {aiFeedback[part.id].feedback.split('•').filter(Boolean).map((point, i) => (
                                                ...
                                                {aiFeedback[part.id].suggestions && (
                                                  <p className="text-neutral-600 dark:text-neutral-400 mt-1 text-xs italic">💡 {aiFeedback[part.id].suggestions}</p>
                                                )}
                                              </div>
                                            )}
                              ...
                              <div className="mt-4 p-4 rounded-lg border bg-neutral-50 dark:bg-neutral-950 border-neutral-200 dark:border-neutral-800">
                                ...
                                {aiFeedback[subQ.id] && (
                                  <div className="text-sm p-3 bg-blue-50 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-900/50 rounded-md mb-3">
                                    <div className="text-neutral-700 dark:text-neutral-300 space-y-1">
                                      {aiFeedback[subQ.id].feedback.split('•').filter(Boolean).map((point, i) => (
                                        <p key={i} className="flex gap-1">
                                          <span>•</span>
                                          <span>{point.trim()}</span>
                                        </p>
                                    ...
                                    {aiFeedback[subQ.id].suggestions && (
                                      <p className="text-neutral-600 dark:text-neutral-400 mt-2 text-xs italic">💡 {aiFeedback[subQ.id].suggestions}</p>
                                    )}
                                  </div>
                                )}
                  ...
                  {!showResults ? (
                    ...
                    <div className="flex flex-col gap-3">
                      <Button
                        ...
                        onClick={() => {
                          if (!currentQuestion) return;
                          const allSubs = currentQuestion.subQuestions.flatMap(sq => {
                            if (sq.parts && sq.parts.length > 0) {
                              return sq.parts.map(p => ({
                                questionTitle: currentQuestion.title,
                                subLabel: `${sq.label}${p.label ? ` ${p.label}` : ""}`,
                                questionText: p.contentBlocks?.find(b => b.type === "text")?.content || p.questionText || "",
                                maxMarks: p.maxMarks || 0,
                                score: subQuestionResults[p.id] || 0,
                                userAnswer: Object.values(userInputs[p.id] || {}).filter(Boolean).join("\n") || "No answer provided",
                                feedback: aiFeedback[p.id]?.feedback,
                                suggestions: aiFeedback[p.id]?.suggestions,
                              }));
                            }
                            return [{
                              questionTitle: currentQuestion.title,
                              subLabel: sq.label,
                              questionText: sq.contentBlocks?.find(b => b.type === "text")?.content || sq.questionText || "",
                              maxMarks: sq.maxMarks || 0,
                              score: subQuestionResults[sq.id] || 0,
                              userAnswer: Object.values(userInputs[sq.id] || {}).filter(Boolean).join("\n") || "No answer provided",
                              feedback: aiFeedback[sq.id]?.feedback,
                              suggestions: aiFeedback[sq.id]?.suggestions,
                            }];
                          });
                          const totalScore = allSubs.reduce((s, i) => s + i.score, 0);
                          const maxScore = allSubs.reduce((s, i) => s + i.maxMarks, 0);
