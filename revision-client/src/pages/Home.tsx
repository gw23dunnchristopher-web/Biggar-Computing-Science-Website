...
export default function Home() {
  ...
  const isLoggedIn = !!student || isTeacherLoggedIn;

  useEffect(() => {
    setIsTeacherLoggedIn(!!localStorage.getItem("teacher_token"));
  }, []);
  ...
  const questionsByTopic = useMemo(() => {
    ...
    questions.forEach(q => { if (grouped[q.topic]) grouped[q.topic].push(q); });
    return grouped;
  }, [questions]);
  ...
  const estimatedStats = useMemo(() => {
    ...
    return { marks: totalMarks, time: Math.ceil(totalMarks * 1.125) };
  ...
  return (
    <div style={{ fontFamily: "Arial, sans-serif", minHeight: "100vh" }}>
      ...
      <div style={{
        ...
        <h1 style={{ fontSize: "22px", fontWeight: "normal" }}>Higher CS Revision Tool</h1>
      </div>
      ...
      <div style={{ display: "flex", marginTop: "150px", marginBottom: "50px", minHeight: "calc(100vh - 200px)" }}>
        ...
        <div style={{
          ...
          <div style={{ padding: "10px", backgroundColor: "#2a2a2a", borderBottom: "1px solid #444" }}>
            <input type="text" placeholder="Search Higher pages..."
              style={{
          ...
          <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
            ...
            <li>
              <a href="/HTML/Higher/HigherHome.html" style={S.sidebarLink}><span>Main Website</span></a>
            </li>
            ...
            {TOPICS.map(topic => (
              <li key={topic.id}>
                <Link href={`/revise/${topic.id}`}>
                  <a style={S.sidebarLink} data-testid={`link-topic-${topic.id}`}>
              ...
              <>
                <li>
                  <div style={{ ...S.sidebarLink, backgroundColor: "#1a3a5c", cursor: "default" }}>
                    <span data-testid="text-student-username">&#128393; {student.username}</span>
                  </div>
                </li>
                <li>
                ...
                <li>
                  <a style={{ ...S.sidebarLink, paddingLeft: "20px" }} onClick={logout} data-testid="button-student-logout">
                    <span>Logout</span>
                  </a>
                </li>
              </>
        ...
        <div style={{ marginLeft: "320px", width: "calc(100% - 320px)", backgroundColor: "white", padding: "10px 10px 60px" }}>
          ...
          <div style={{ maxWidth: "1400px", margin: "auto", padding: "10px", textAlign: "left" }}>
            <p style={{ textAlign: "center", fontSize: "18px", padding: "10px" }}>
              <b>Use the sidebar to select a topic or action to get started with your Higher Computing Science revision!</b>
            </p>
                ...
                {TOPICS.map(topic => (
                  <tr key={topic.id}>
                    ...
                    <td style={{ ...S.td, textAlign: "center" } as React.CSSProperties}>
                      {loading ? "..." : `${questionsByTopic[topic.id as Topic]?.length || 0}`}
                    </td>
                    <td style={{ ...S.td, textAlign: "center" } as React.CSSProperties}>
                      <Link href={`/revise/${topic.id}`}>
            ...
            {publishedExams.length > 0 && (
              <>
                <h2 style={S.h2 as React.CSSProperties}>Additional Exams</h2>
                <p style={{ fontSize: "18px", padding: "10px" }}>Teacher-created exams to help you prepare.</p>
                <table style={{ borderCollapse: "collapse", margin: "20px auto", width: "100%", border: "1px solid black" }}>
                  <thead>
                    <tr>
                      <th style={S.th as React.CSSProperties}>Exam</th>
                      <th style={S.th as React.CSSProperties}>Questions</th>
                      <th style={S.th as React.CSSProperties}>Est. Time</th>
                      <th style={S.th as React.CSSProperties}>Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    {publishedExams.map(exam => {
                      ...
                      return (
                        <tr key={exam.id} data-testid={`card-additional-exam-${exam.id}`}>
                          <td style={{ ...S.td, fontWeight: "bold" } as React.CSSProperties}>{exam.title}</td>
                          <td style={{ ...S.td, textAlign: "center" } as React.CSSProperties}>{info.count}</td>
                          <td style={{ ...S.td, textAlign: "center" } as React.CSSProperties}>~{Math.ceil(info.marks * 1.125)} min</td>
                          <td style={{ ...S.td, textAlign: "center" } as React.CSSProperties}>
                            <Link href={`/timed-exam/additional/${exam.id}`}>
                              <a style={{ color: "#17479b", textDecoration: "underline", fontWeight: "bold" }}>Start &#9654;</a>
                            </Link>
                          </td>
                        </tr>
                      );
            ...
            <p style={{ fontSize: "18px", padding: "10px" }}>
              This revision tool provides access to Higher Computing Science past paper questions with AI-powered marking and feedback.
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="flex items-center justify-between text-sm">
                <span className="text-neutral-600">Estimated marks:</span>
