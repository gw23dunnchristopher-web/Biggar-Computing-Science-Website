...
export default function QuestionEditor() {
  ...
  const handlePreviewGrade = useCallback(async (subQ: SubQuestion, parentSubQ?: SubQuestion) => {
    ...
    const studentDiagramImage = previewImages[subQ.id] || "";
    if (!studentAnswer && !studentDiagramImage) {
    ...
    try {
      const referenceFiles = subQ.markingGuidanceData?.exampleFiles || [];
            ...
            {formData.subQuestions.map((subQ, index) => (
                <AccordionItem key={subQ.id || index} value={`question-${index}`} className="border rounded-lg bg-white dark:bg-neutral-900">
                    ...
                    <AccordionContent className="px-4 pb-4">
                        ...
                        {(subQ.inputStyle === "image-paste" || subQ.inputStyle === "drawing" || subQ.inputStyle === "erd-annotation" || subQ.inputStyle === "nav-structure" || subQ.inputStyle === "nav-structure-higher" || subQ.inputStyle === "tag-matching" || subQ.inputStyle === "structure-dataflow" || subQ.inputStyle === "form-wireframe" || subQ.inputStyle === "structure-diagram" || subQ.inputStyle === "entity-occurrence-diagram") && (
